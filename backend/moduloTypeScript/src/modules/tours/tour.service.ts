...
export class TourService {
  ...
  async findAll(): Promise<any[]> {
    ...
    // Enriquecer con datos de otros servicios
    const toursEnriquecidos = await Promise.all(
      tours.map(async (tour) => {
        const destino = await httpClient.getDestino(tour.id_destino);
        const servicios = tour.id_servicios ? await httpClient.getServicios(tour.id_servicios) : [];
  ...
  async create(createTourDto: CreateTourDto): Promise<Tour> {
    ...
    // Validar que el destino existe (llamada al servicio Python)
    const destino = await httpClient.getDestino(createTourDto.id_destino);
    if (!destino) {
      throw new Error('El destino especificado no existe');
    }
  ...
  async update(id: number, updateTourDto: UpdateTourDto): Promise<Tour> {
    ...
    // Si se actualiza el destino, validar que existe
    if (updateTourDto.id_destino) {
      const destino = await httpClient.getDestino(updateTourDto.id_destino);
      if (!destino) {
        throw new Error('El destino especificado no existe');
      }
    }
